...
const Articles = () => {
  ...
  const articles: ArticleTile[] = [
    ...
    },
    {
        ...
        img: pic3,
        shortV: 'Każdy przedsiębiorca w Polsce musi pamiętać o kluczowych datach w roku podatkowym, aby uniknąć kar i opóźnień. Poniżej znajdziesz przewodnik po najważniejszych terminach, które powinny znaleźć się w Twoim kalendarzu biznesowym.',
        fullArt: ''
    },
    {
        id: 4,
        ...
        date: '2.02.2024',
        title: 'Cyfryzacja Twojej firmy - od papierkowej dokumentacji do zarządzania online',
        img: pic4,
        shortV: 'W dzisiejszym szybko rozwijającym się świecie biznesu, cyfryzacja stała się kluczowa dla efektywności i wzrostu. Przejście od tradycyjnych, papierowych metod zarządzania do cyfryzacji procesów biznesowych może wydawać się wyzwaniem, ale jest niezbędne dla nowoczesnego przedsiębiorstwa. W tym artykule dowiesz się, jak skutecznie dokonać tej transformacji.',
        fullArt: ''
    },
    {
        id: 5,
      ...
      <div className="flex flex-row justify-start mx-[5%] font-poppins flex-wrap">
          {articles.map((x)=>(
            <div className="w-1/4 mx-10 flex-col flex my-20" key={x.id}>
                ...
                <h1 className="text-xl font-black mt-10 border-b-2 pb-3 ">{x.title}</h1>
                <img src={x.img} alt="article thumbnail" className="my-5" />
                <p className=" text-justify">{x.shortV}</p>
                <p>
                    <Link to='' className="flex flex-row items-center font-bold mt-5 hover:text-green-700">Czytaj dalej <FaArrowRight className='ml-1' /></Link>
                </p>
            </div>
          ))}
      </div>
